  ...
  const load = useCallback(async (options) => {
    ...
    return {
      ...
      teams: constructorStandings.map((standing) => {
        ...
        return { slug: constructor.constructorId, name: constructor.name, position: Number(standing.position), points: Number(standing.points), wins: Number(standing.wins || 0) };
      }),
      races
    };
  }, [season]);
